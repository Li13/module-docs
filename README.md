...
没有路径开头则视为导入一个包
会先判断`moduleA`是否是一个**核心模块** 如`path`，`http`，优先导入**核心模块**
...
`Node`的模块 实际上可以理解为代码被包裹在一个`函数包装器`内
一个简单的`require demo`：
...
`export` 和 `import` 只能出现在代码的顶层，下面这段语法是**错误**的
...
`import` 的导入名不能为字符串或在判断语句，下面代码是**错误**的
...
使用 `import` 被导入的变量是与原变量**绑定/引用**的，可以理解为 `import` 导入的变量无论是否为基本类型都是引用传递
...
如果你使用 `node`，可以在 `.mjs` 文件使用 `ES6`，也支持 `CommonJs` 查看 [nodejs es-modules.md][5]
...
可以在 [es6module example][10] 仓库中获取代码在本地进行测试验证

### 浏览器中使用 ###

你需要起一个`Web服务器`来访问，双击本地运行 `index.html` 并不会执行 `type=module` 标签
...
因为 `ES6 Module` 在浏览器中兼容并不是很好[兼容性表][11]，这里就不介绍浏览器支持情况了，我们一般不会直接在浏览器中使用

### Nodejs中使用 ###

[nodejs es-modules.md][6]
...
（上面链接可以知道 `ES Module`的状态是 `DRAFT`， 属于起草阶段）
...
可以看到 `es module` 在`浏览器`与`node`中**兼容性差**与**实验功能**的
我们大多时候在 **打包工具** 中使用
...
这时就提出了在 `package.json` 加一个 `module` 的字段来指向 `es module`规范的文件，`main -> CommonJs`，那么`module - es module` [pkg.module][9]
...
- [ECMAScript® 2015 Language Specification sec-imports/sec-exports](http://www.ecma-international.org/ecma-262/6.0/#sec-imports)
...
- [nodejs docs modules](https://nodejs.org/dist/latest-v10.x/docs/api/modules.html#modules_modules)
...
[5]:https://github.com/nodejs/node-eps/blob/master/002-es-modules.md#46-es-consuming-commonjs
[6]:https://github.com/nodejs/node-eps/blob/master/002-es-modules.md
[7]:https://www.rollupjs.com/repl?version=0.68.2&shareable=
[8]:https://leanpub.com/understandinges6/read
[9]:https://github.com/rollup/rollup/wiki/pkg.module
[10]:https://github.com/Li13/module
[11]:https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/import#%E6%B5%8F%E8%A7%88%E5%99%A8%E5%85%BC%E5%AE%B9
